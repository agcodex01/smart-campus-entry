...
def index():
    ...
    entries = db.execute(
        "SELECT e.id, created, u.first_name, u.last_name, u.course,  u.year_level, u.student_id   FROM entries e JOIN users u ON e.user_id = u.id  ORDER BY created DESC LIMIT 4"
    ).fetchall()
